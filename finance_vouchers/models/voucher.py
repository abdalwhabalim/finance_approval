...
class CashVoucher(models.Model):
    ...
    team_custom_id = fields.Many2one(
        comodel_name="voucher.team.custom",required=True, string="Voucher Team", domain="[('company_id', '=', company_id)]",
        readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, ondelete="restrict"
    )

    approver_ids = fields.One2many(
        comodel_name="voucher.approver", inverse_name="order_id", string="Approvers", readonly=True)

    current_approver = fields.Many2one(
        comodel_name="voucher.approver", string="Approver",
        compute="_compute_approver", store=True, compute_sudo=True)

    next_approver = fields.Many2one(
        comodel_name="voucher.approver", string="Next Approver",
        compute="_compute_approver", store=True, compute_sudo=True)
    ...
    def _track_subtype(self, init_values):
        ...
        return super(CashVoucher, self)._track_subtype(init_values)

    def button_approve(self, force=False):
        for order in self:
            if not order.team_custom_id:
                # Do default behaviour if SO Team is not set
                super(CashVoucher, order).button_approve(force)
                ...
                if order.current_approver.user_id == self.env.user or self.env.is_superuser():
                    ...
                    order.message_post(body=_('Voucher approved by %s') % self.env.user.name)
                    ...
                    if order.next_approver:
                        ...
                        order.message_post_with_view(
                            'finance_vouchers.order_approval',
                            subject=_('Voucher Approved: %s') % (order.name,),
                        ...
                        return super(CashVoucher, order).confirm_post()
    ...
    def confirm_manager(self):
        for order in self:
            ...
            if not order.team_custom_id:
                # Do default behaviour if SO Team is not set
                super(CashVoucher, order).button_confirm()
            else:
                ...
                if order.next_approver:
                    ...
                    super(CashVoucher, order).confirm_post()
        ...
        for order in self:
            ...
            for team_approver in order.team_custom_id.approver_ids:
                ...
                min_amount = team_approver.company_currency_id._convert(
                    ...
                    order.currency_id,
                    order.company_id,
                    order.request_date or fields.Date.today())
                ...
                # raise ('jjjjjjjjjjj')
                self.env['voucher.approver'].create({
    ...
    def send_to_approve(self):
        for order in self:
            ...
            if not order.next_approver:
                reason_msg = _("There are no approvers in the selected Voucher team.")
                raise UserError("%s %s" % (main_error_msg, reason_msg))
            ...
            order.with_user(order.user_id).message_post_with_view(
                'finance_vouchers.request_to_approve_so',
                subject=_('Voucher Approval: %s') % (order.name,),
                composition_mode='mass_mail',
    ...
    def _check_lock_amount_total(self):
        ...
        for order in self:
            ...
            if order.team_custom_id.lock_amount_total:
                ...
                suggestion = _('To make changes, cancel and reset Voucher to draft. ')
                raise UserError(msg + "\n\n" + reason + "\n\n" + suggestion)