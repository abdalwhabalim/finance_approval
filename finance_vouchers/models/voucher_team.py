...
class SaleTeam(models.Model):
    _name = "voucher.team.custom"
    _description = "voucher Team"
    ...
    approver_ids = fields.One2many('voucher.team.approver','team_custom_id', string='Approvers')

    @api.constrains('company_id')
    def _check_team_company(self):
        for team in self:
            if team.company_id.voucher_approval_route == 'no':
...
class SaleTeamApprover(models.Model):
    _name = "voucher.team.approver"
    _description = "voucher Team Approver"
    _order = 'sequence'
    _rec_name = 'sequence'

    sequence = fields.Integer(string='Sequence', default=10)

    team_custom_id = fields.Many2one('voucher.team.custom', string='Team', ondelete='cascade')
...
class SaleOrderApprover(models.Model):
    _name = "voucher.approver"
    _inherit = "voucher.team.approver"
    _description = "voucher Approver"
    _order = 'state asc'

    team_approver_id = fields.Many2one(
        comodel_name='voucher.team.approver', string='voucher Team Approver',
        ondelete='set null')

    order_id = fields.Many2one(
        comodel_name='cash.voucher', string='voucher',
